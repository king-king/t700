        ...
        el.bindEvent( "touchstart", function ( start ) {
            ...
            var moveHandler = document.bindEvent( "touchmove", function ( move ) {
                if ( Math.pow( move.touches[0].pageX - spx, 2 ) + Math.pow( move.touches[0].pageY - spy, 2 ) > 9 ) {
                    if ( !isGet && !isMoving && arg.y &&
                        Math.pow( move.touches[0].pageX - spx, 2 ) + Math.pow( move.touches[0].pageY - spy, 2 ) > 50 ) {
                        if ( Math.abs( move.touches[0].pageX - spx ) < Math.abs( move.touches[0].pageY - spy ) ) {
                            isMoving = true;
                        }
                        isGet = true;
                    }
